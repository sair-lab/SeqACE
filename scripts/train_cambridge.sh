...
testing_exe="${REPO_PATH}/test_ace_feature.py"
...
for scene in ${scenes[*]}; do
  taskset -c 33-48 python $training_exe "$datasets_folder/$scene" "$out_dir/$scene.pt"
  taskset -c 33-48 python $testing_exe --test_mode 0 "$datasets_folder/$scene" "$out_dir/$scene.pt" 2>&1 | tee "$out_dir/log_${scene}.txt"
done
