...
for scene in ${scenes[*]}; do
  ...
  for cluster_idx in $(seq 0 $((num_clusters-1))); do
    ...
    python $testing_exe "$datasets_folder/$scene" "$net_file" --test_mode 0 --session "${cluster_idx}_${num_clusters}" 2>&1 | tee "${current_out_dir}/test_${cluster_idx}_${num_clusters}.txt"
  done
