...
class CamLocDataset(Dataset):
    ...
    def __init__(self,
        ...
        keypoint_dir = root_dir / 'matching'
        ...
        # Load keypoits and matching results
        if self.load_kpts:
            self.keypoint_files = sorted(keypoint_dir.iterdir())
    ...
    @staticmethod
    def _to_opencv(image):
        image_cv = np.array(image)[:, :, ::-1].astype(np.float32)
        gray_image = cv2.cvtColor(image_cv, cv2.COLOR_BGR2GRAY)
        return gray_image

    @staticmethod
    def _rotate_pil_image(image, angle, order, mode='constant'):
        image = rotate(image, angle, order=order, mode=mode)
        return image
    ...
    def _get_single_item(self, idx, image_height):
        ...
        if self.load_kpts:
            ...
            track_info = np.loadtxt(self.keypoint_files[idx])
            track_info = torch.from_numpy(track_info)
            keypoints = track_info[1:, 1:]
            keypoints = f_scale_factor * keypoints
        ...
        if self.augment:
            ...
            # original_image = self._rotate_pil_image(original_image, angle, 1, 'reflect')
            ...
            if self.load_kpts:
                ...
                center = torch.tensor([image.size(2), image.size(1)], dtype=torch.float32) / 2
                translated_keypoints = keypoints - center  
                rotated_keypoints = torch.matmul(translated_keypoints, rotation_matrix)  
                keypoints = rotated_keypoints + center

        if self.load_kpts:
        # keypoint mask
            track_info[1:, 1:] = keypoints
            W, H, N = image.size(2), image.size(1), track_info.shape[0]
            padded_keypoints = torch.full((1025, 3), -1, dtype=torch.float32)
            padded_keypoints[:N, :] = track_info
            keypoint_mask = (keypoints[:, 0] > 0) & (keypoints[:, 0] < W-1) & (keypoints[:, 1] > 0) & (keypoints[:, 1] < H-1)
            keypoint_mask = torch.cat((torch.tensor([True]), keypoint_mask, torch.tensor([False]*(1025-N))))

        # original_image = self._to_opencv(original_image)
        ...
        original_image = cv2.imread(str(self.rgb_files[idx]), cv2.IMREAD_GRAYSCALE)
        if original_image.shape[0] != image_height:
            original_image = cv2.resize(original_image, None, fx=f_scale_factor, fy=f_scale_factor)

        if self.load_kpts:
            return image, image_mask, pose, pose_inv, intrinsics, intrinsics_inv, coords, str(self.rgb_files[idx]), original_image, padded_keypoints, keypoint_mask
        else:
            return image, image_mask, pose, pose_inv, intrinsics, intrinsics_inv, coords, str(self.rgb_files[idx]), original_image
